                                    ...
                                    <div class="input-group">
                                        ...
                                        <select class="select2-basic" name="location" id="location">
                                            ...
                                            @foreach ($locations as $location)
                                                <option
                                                    value="{{ $location->id }}" {{ $request->location == $location->id ? 'selected' : '' }}>{{ __($location->name) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-xl-2 col-md-6">
                                    <label>@lang('Mini budget')</label>
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="las la-money-bill-wave"></i></span>
                                        <input type="text" name="date" placeholder="50,000" class=" form--control"
                                               autocomplete="off">
                                    </div>
                                </div>
                                <div class="col-xl-2 col-md-6">
                                    <label>@lang('Max budget')</label>
                                    <div class="input-group">
                                        <span class="input-group-text"><i class="las la-money-bill-wave"></i></span>
                                        <input type="text" name="date" placeholder="250,000" class=" form--control"
                                               autocomplete="off">
                                    </div>
                                </div>
                                <input type="hidden" name="date" placeholder="50,000 - 250,000" id="date"
                                       data-range="true" data-multiple-dates-separator=" - " data-language="en"
                                       class="datepicker-here form--control" placeholder="M/D/Y - M/D/Y"
                                       autocomplete="off">
                        ...
                        <div class="action-sidebar">
                            ...
                            <div class="action-widget">
                                ...
                                <div class="action-widget__body">
                                    <select class="select select-sm propertyType">
                                        ...
                                        @foreach ($propertyTypes as $propertyType)
                                            <option
                                                value="{{ $propertyType->id }}" {{ $request->propertyType == $propertyType->id ? 'selected':'' }}>{{ __($propertyType->name) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="action-widget">
                                ...
                                <div class="action-widget__body">
                                    <div class="row">
                                        <div class="col-6">
                                            <input type="number" placeholder="min" min="0" step="any"
                                                   class="form--control form-control-sm min_price">
                                        </div>
                                        <div class="col-6">
                                            <input type="number" placeholder="max" min="0" step="any"
                                                   class="form--control form-control-sm max_price">
                                        </div>
                                        <div class="col-12 mt-3">
                                            <button type="submit" class="btn btn-sm btn--base w-100"
                                                    id="filter_btn">@lang('Filter')</button>
                                        </div>
                                    </div>
                                </div>
                            </div><!-- action-widget end -->
                                        ...
                                        <div class="form-check custom--checkbox d-flex justify-content-between pe-2">
                                            <div class="left">
                                                <input class="form-check-input amenity-check" type="checkbox"
                                                       value="{{ $amenity->id }}" id="checkbox-{{ $amenity->id }}">
                            ...
                            @forelse ($properties as $property)
                                ...
                                <div class="hotel-card">
                                    ...
                                    <div class="hotel-card__thumb">
                                        <a href="{{ route('property', [$property->id, slug($property->name)]) }}"
                                           class="d-block"><img
                                    ...
                                    <div class="hotel-card__content">
                                        @if ($property->rating >= 4)
                                            <span
                                                class="fs--12px bg--primary text-white px-2 rounded-2 mb-2">@lang('Top Rated')</span>
                                        @endif
                                        @if ($property->top_reviewed)
                                            <span
                                                class="fs--12px bg--success text-white px-2 rounded-2 mb-2">@lang('Best Reviewed')</span>
                                        @endif
                                        <h3 style="justify-content: space-between" class="d-flex  title">
                                            <a class=""
                                               href="{{ route('property', [$property->id, slug($property->name)]) }}">{{ __($property->name) }}</a>
                                            @php
                                                $wCk = checkWishList($property->id);
                                            @endphp
                                            <a href="javascript:void(0)" title="@lang('Add To Wishlist')"
                                               class="add-to-wish-list {{$wCk?'active':''}}"
                                               data-id="{{$property->id}}">
                                                @if($wCk)
                                                    <i style="color: #3059B8 " class="las la-heart"></i>
                                                @else
                                                    <i style="color: #3059B8 " class="lar la-heart"></i>
                                                @endif
                                            </a>
                                        </h3>
                                        <p class="mt-1"><i
                                                class="las la-map-marker-alt fs--18px"></i> {{ __($property->location->name) }}
                                        </p>
                                        <ul class="features-list mt-2">
                                            @if($property->extra_features)
                                                @foreach($property->extra_features as $feature)
                                                    @if($loop->iteration == 3)
                                                        <li>@lang('And More')</li> @break
                                                    @endif
                                                    <li>{{ __($feature) }}</li>
                                                @endforeach
                                            @endif
                                        ...
                                        @if($general->pv == 1)
                                            <div class="hotel-rating fs--14px mt-3">
                                                <span
                                                    class="amount bg--success fs--16px px-2 text-white rounded-2">{{ showAmount($property->rating) }}</span>
                                    ...
                                    <div class="hotel-card__action">
                                        <div class="bottom w-100">
                                            <div class="price text--base">
                                                ...
                                                <span
                                                    class="{{ $highestPrice > 9999 ? 'd-block' : '' }}">{{ $general->cur_sym.showAmount($property->property_amount) }}</span>
                                            </div>
                                            <p class="fs--14px mt-1">@lang('price per year')</p>
                                            <a href="{{ route('property', [$property->id, slug($property->name)]) }}"
                                               class="btn btn-sm btn--base mt-3 w-100">@lang('View Details')</a>
                                        </div>
                                    </div>
                                </div><!-- hotel-card end -->
                            @empty
