    ...
    <div class="page-wrapper default-version">
        <div class="form-area bg_img" data-background="{{asset('assets/laramin/images/1.jpg')}}">
            <div class="form-wrapper">
                ...
                <form action="{{ route('Bloggers.login') }}" method="POST" class="cmn-form mt-30">
                    @csrf
                    ...
                    @include('bloggers.partials.custom_captcha')
                    <div class="form-group d-flex justify-content-between align-items-center">
                        <a href="{{ route('Bloggers.password.request') }}" class="text-muted text--small"><i class="las la-lock"></i>@lang('Forgot password?')</a>
                    </div>
                    <div class="form-group">
                ...
                <div class="text-center">
                   <span class="text-dark"> @lang('Don\'t have an account?') </span> <a href="{{ route('Bloggers.register') }}">@lang('Register now.')</a>
                </div>
            </div>
        </div><!-- login-area end -->
    </div>
