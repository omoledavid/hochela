            ...
            <div class="form-wrapper">
                ...
                <form action="{{ route('owner.login') }}" method="POST" class="cmn-form mt-30">
                    ...
                    <div class="form-group d-flex justify-content-between align-items-center">
                        <a href="{{ route('owner.password.request') }}" class="text-muted text--small"><i
                                class="las la-lock"></i>@lang('Forgot password?')</a>
                    </div>
                    <div class="form-group">
                        <button type="submit" style="border-radius: 10px" class="submit-btn mt-25">@lang('Login') <i
                                class="las la-sign-in-alt"></i>
                        </button>
                        <a href="{{ route('google.login.owner') }}" style="background: #c32828; border-radius: 10px"
                           class="submit-btn mt-25">@lang('Google Login') <i
                                class="lab la-google-plus-g"></i>
                        </a>
                    </div>
                </form>
                <div class="text-center">
                    <span class="text-dark"> @lang('Don\'t have an account?') </span> <a
                        href="{{ route('owner.register') }}">@lang('Register now.')</a>
                </div>
            </div>
        </div><!-- login-area end -->
    </div>
