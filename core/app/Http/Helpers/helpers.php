...
function getLatestVersion()
{
    ...
    $url = 'https://license.smavotex.com/updates/version/' . systemDetails()['name'];
...
function getTemplates()
{
    $param['purchasecode'] = env("PURCHASECODE");
    $param['website'] = @$_SERVER['HTTP_HOST'] . @$_SERVER['REQUEST_URI'] . ' - ' . env("APP_URL");
    $url = 'https://license.smavotex.com/updates/templates/' . systemDetails()['name'];
    $result = curlPostContent($url, $param);
    if ($result) {
        return $result;
    } else {
...
function ratings($number)
{
    ...
    for ($i = 1; $i <= 5; $i++) {
        if ($review < $i) {
            ...
            $html .= '<i class="las la-star fill gold-star"></i>';
        }
    }
    return $html;
}
