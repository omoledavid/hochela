...
class PropertyController extends Controller
{
    public function properties(){
        ...
        $properties = Property::with('propertyType', 'location', 'rooms', 'roomCategories')->where('bloggers_id', Auth::guard('bloggers')->id())->orderBy('id', 'DESC')->paginate(getPaginate());
        return view('bloggers.property.index', compact('pageTitle', 'emptyMessage', 'properties'));
    }
    ...
    public function create(){
        ...
        return view('bloggers.property.create', compact('pageTitle', 'propertyTypes', 'locations', 'amenities'));
    }

    public function edit($id){
        $pageTitle = 'Update Property';
        $property = Property::where('bloggers_id', Auth::guard('bloggers')->id())->findOrFail($id);
        $propertyTypes = PropertyType::where('status', 1)->get();
        $locations = Location::where('status', 1)->get();
        $amenities = Amenity::where('status', 1)->get();
        return view('bloggers.property.edit', compact('pageTitle', 'property', 'propertyTypes', 'locations', 'amenities'));
    ...
    public function saveProperty(Request $request, $id=0){
        $request->validate([
            ...
            $property->bloggers_id = Auth::guard('bloggers')->id();
