...
class PaymentController extends Controller
{
    ...
    public function deposit()
    {
        ...
        return view($this->activeTemplate . 'user.payment.deposit', compact('gatewayCurrency', 'pageTitle','checkOutData'));
    }
    ...
    public function depositInsert(Request $request)
    {
        ...
        $data->booked_property_id = $bookedProperty->id;
    ...
    public static function userDataUpdate($trx)
    {
        ...
        if ($data->status == 0) {
            ...
            $bookedProperty = BookedProperty::find($data->booked_property_id);
            ...
            $owner = $bookedProperty->property->owner;
            $owner->balance += $data->amount;
            $owner->save();

            BookedRoom::where('booked_property_id', $data->booked_property_id)->update([
                'status'=>1
            ]);
            ...
            notify($owner, 'PROPERTY_BOOKED', [
                'property_name' => $property->name,
                'user_name' => $user->fullname,
                'check_in_date' => $bookedProperty->date_from,
                'check_out_date' => $bookedProperty->date_to,
                'trx' => $trx,
                'amount' => showAmount($data->amount),
                'currency' => $general->cur_text,
    ...
    public function manualDepositUpdate(Request $request)
    {
        ...
        $notify[] = ['success', 'Your deposit request has been submited.'];
