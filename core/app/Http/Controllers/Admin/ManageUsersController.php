...
class ManageUsersController extends Controller
{
    public function allUsers()
    {
        ...
        $ref_owners = false;
        return view('admin.users.list', compact('pageTitle', 'emptyMessage', 'users', 'ref_owners'));
    }
    ...
    public function usersWithReferrer()
    {
        $pageTitle = 'Users with referrer';
        $emptyMessage = 'No user with referrer found';

        // Query users with at least one referrer
        $users = User::withCount('referrers') // Count the number of referrers
        ->having('referrers_count', '>', 0) // Filter users with at least 1 referrer
    ...
    public function search(Request $request, $scope)
    {
        ...
        $ref_owners = false;
        return view('admin.users.list', compact('pageTitle', 'search', 'scope', 'emptyMessage', 'users', 'ref_owners'));
    }
    ...
    public function detail($id)
    {
        ...
        $total_referred = User::query()->where('ref_username', $user->username)->count() + Owner::query()->where('ref_username', $user->username)->count();
        return view('admin.users.detail', compact('pageTitle', 'user', 'totalDeposit', 'totalTickets', 'totalBookedProperties', 'countries', 'total_referred'));
    }

    public function refUsers($id)
    {
        $user = User::findOrFail($id);
        $pageTitle = 'Referred Users';
        $emptyMessage = 'No Referred user found';
        $users = User::where('ref_username', $user->username)->orderBy('id', 'desc')->paginate(getPaginate());
        $owners = Owner::where('ref_username', $user->username)->orderBy('id', 'desc')->paginate(getPaginate());
        if ($owners->count() > 0) {
            $ref_owners = true;
        } else {
            $ref_owners = false;
        }
        return view('admin.users.list', compact('pageTitle', 'emptyMessage', 'users', 'owners', 'ref_owners'));
    }
    ...
    public function bookedProperties(Request $request, $id)
    {
        ...
        return view('admin.users.booked_properties', compact('pageTitle', 'emptyMessage', 'bookedProperties'));
    }
