...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        if ($admin->level == 1) {
            ...
            $widget['verified_owners'] = Owner::where('status', 1)->count();
            $widget['email_unverified_owners'] = Owner::where('ev', 0)->count();
            $widget['sms_unverified_owners'] = Owner::where('sv', 0)->count();

            //blogger Info
            $widget['total_bloggers'] = Bloggers::count();
            $widget['verified_bloggers'] = Bloggers::where('status', 1)->count();
            $widget['email_unverified_bloggers'] = Bloggers::where('ev', 0)->count();
            $widget['sms_unverified_bloggers'] = Bloggers::where('sv', 0)->count();
            ...
            return view('admin.dashboard', compact('pageTitle', 'widget', 'report', 'withdrawals', 'chart', 'payment', 'paymentWithdraw', 'depositsMonth', 'withdrawalMonth', 'months', 'deposits', 'widget'));
        }elseif($admin->level == 2){

            //News
            $widget['total_news'] = News::where('user_id', $admin->id)->count();
            $widget['pending_news'] = News::where('user_id', $admin->id)->pending()->count();
            $widget['approved_news'] = News::where('user_id', $admin->id)->approved()->count();
            $widget['rejected_news'] = News::where('user_id', $admin->id)->rejected()->count();

            $news = News::where('user_id', $admin->id)->with('category')->latest()->take(10)->get();
            return view('admin.blog.dashboard', compact('pageTitle', 'widget', 'news', 'admin'));
        }
    ...
    public function requestReport()
    {
        ...
        $url = "https://license.smavotex.com/issue/get?" . http_build_query($arr);
    ...
    public function reportSubmit(Request $request)
    {
        ...
        $url = 'https://license.smavotex.com/issue/add';
